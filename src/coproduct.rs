...
use frunk::traits::{Func, Poly, ToMut, ToRef};
#[cfg(feature = "serde")]
...
impl<Head, Tail> Coproduct<Head, Tail> {
    ...
    pub fn subset<Targets, Indices>(
        ...
        Targets,
        <Self as CoproductSubsetter<Targets, Indices>>::Remainder,
    >
    where
    ...
    /// * A single closure (for a Coproduct that is homogeneous).
    /// * A single [`Poly`].
    ///
    ...
    /// * A single closure (for mapping a Coproduct that is homogeneous).
    /// * A single [`Poly`].
    ///
    /// # Examples
    ///
...
impl<Head, I, Tail, TailIndex> CoprodInjector<I, There<TailIndex>>
    ...
    Tail: CoprodInjector<I, TailIndex>,
{
    fn inject(to_insert: I) -> Self {
        let tail_inserted =
            <Tail as CoprodInjector<I, TailIndex>>::inject(to_insert);
...
impl<Head, FromTail, Tail, TailIndex>
    CoproductSelector<FromTail, There<TailIndex>> for Coproduct<Head, Tail>
where
...
impl<Head, FromTail, Tail, TailIndex>
    CoproductTaker<FromTail, There<TailIndex>> for Coproduct<Head, Tail>
where
...
impl<F, R, MapperTail, CH, CTail> CoproductMappable<HCons<F, MapperTail>>
    for Coproduct<CH, CTail>
where
    F: FnOnce(CH) -> R,
...
{
    type Output =
        Coproduct<R, <CTail as CoproductMappable<MapperTail>>::Output>;
...
impl<'a, F, R, MapperTail, CH, CTail>
    CoproductMappable<&'a HCons<F, MapperTail>> for Coproduct<CH, CTail>
where
    F: Fn(CH) -> R,
    CTail: CoproductMappable<&'a MapperTail>,
{
    type Output =
        Coproduct<R, <CTail as CoproductMappable<&'a MapperTail>>::Output>;
...
impl<'a, F, R, MapperTail, CH, CTail>
    CoproductMappable<&'a mut HCons<F, MapperTail>> for Coproduct<CH, CTail>
where
    F: FnMut(CH) -> R,
    CTail: CoproductMappable<&'a mut MapperTail>,
{
    type Output =
        Coproduct<R, <CTail as CoproductMappable<&'a mut MapperTail>>::Output>;
    ...
    CTail: CoproductMappable<Poly<P>>,
{
    type Output = Coproduct<
        <P as Func<CH>>::Output,
        <CTail as CoproductMappable<Poly<P>>>::Output,
    >;
    ...
    CTail: CoproductMappable<&'a Poly<P>>,
{
    type Output = Coproduct<
        <P as Func<CH>>::Output,
        <CTail as CoproductMappable<&'a Poly<P>>>::Output,
    >;
...
impl<'a, P, CH, CTail> CoproductMappable<&'a mut Poly<P>>
    for Coproduct<CH, CTail>
where
    P: Func<CH>,
...
impl<Choices, THead, TTail, NHead, NTail, Rem>
    CoproductSubsetter<Coproduct<THead, TTail>, HCons<NHead, NTail>>
    for Choices
where
...
mod tests {
    ...
    fn test_coproduct_map_with_ref_mapper() {
        ...
        let mapper =
            poly_fn!(|n: i32| -> i32 { n + 3 }, |b: bool| -> bool { !b });
